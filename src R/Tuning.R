...
TuningForecast = function(dependent_var, explanatory_vars, method, lambdaList, alphaList) {
  totalError <- 0
  numberOfWindows <- 0
  ...
  for (alpha in alphaList) {
    for (lambda in lambdaList) {
      ...
      if (method == "Lasso") {
        ...
        model <- glmnet(x_train, y_train, penalty.factor = weights, alpha = 1, lambda = 10000)
      } else {
...
RollingWindowTuningPenalized = function(dependent_var, explanatory_vars, method, lambdaList, alphaList) {
  totalError <- 0
  numberOfWindows <- 0
  ...
  while (endTime + 1 <= nrow(explanatory_vars)) {
    ...
    # je roept dit nu niet aan met lag ??
    totalStats <- CreateDataSetNew(dependent_var, explanatory_vars, beginTime = beginTime, endTime = endTime)
    ...
    for(lambda in lambdaList) {
      for (alpha in alphaList) {
        ...
        if (method == "Lasso") {
          ...
          # Fit Adaptive Lasso model with calculated penalty factors
          model <- glmnet(x_train, y_train, alpha = 1, lambda = lambda, penalty.factor = weights)
        } else {
          stop("Invalid model name provided. Try Lasso, Ridge, ElasticNet, PCA, SPCA, AR, or AdaptiveLasso")
        }
